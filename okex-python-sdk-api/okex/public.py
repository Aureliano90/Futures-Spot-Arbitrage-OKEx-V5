...
class PublicAPI(Client):
    ...
    async def async_get_specific_instrument(self, instType, instId, uly=''):
        """获取单个可交易产品的信息\n
        ...
        if uly:
            params['uly'] = uly
        data = (await self.async_request_with_params(GET, GET_INSTRUMENTS, params))['data']
        return data[0]

    def get_specific_instrument(self, instType, instId, uly=''):
        """获取单个可交易产品的信息\n
        ...
        if uly:
            params['uly'] = uly
        data = self._request_with_params(GET, GET_INSTRUMENTS, params)['data']
        return data[0]

    async def async_get_funding_time(self, instId: str) -> dict:
        """获取当前资金费率\n
    ...
    async def async_get_historical_funding_rate(self, instId: str, after='', before='', limit='') -> List[dict]:
        """获取最近3个月的历史资金费率\n
    ...
    async def async_get_specific_ticker(self, instId: str) -> dict:
        """获取单个产品行情信息\n
